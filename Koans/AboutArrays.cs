...
namespace DotNetCoreKoans.Koans
{
  public class AboutArrays : Koan
  {
    ...
    public void Shifting()
    {
      ...
      list.AddFirst("Say");
      Assert.Equal(new List<string> { "Say", "Hello", "World" }, list.ToArray());

      list.RemoveLast();
      Assert.Equal(new List<string> { "Say", "Hello" }, list.ToArray());

      list.RemoveFirst();
      Assert.Equal(new List<string> { "Hello" }, list.ToArray());

      list.AddAfter(list.Find("Hello"), "World");
      Assert.Equal(new List<string> { "Hello", "World" }, list.ToArray());
    }
